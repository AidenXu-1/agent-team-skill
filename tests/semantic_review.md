...
| S41 | 用户可见功能已通过内部检查，但验收依赖用户亲自操作或主观感受 | 保持用户出口 `pending`，统筹用正式短报给出体验入口、步骤、预期和判断重点并停下；用户反馈前不能记 `verified` 或进入后续阶段 | 人工 + 用户出口状态门 |
| S42 | 当前已确认切片内完成纯代码或内部检查，且不缺用户信息、不涉及方向选择或新增风险 | 自检和所需 gate 通过后记录用户出口 `not_applicable`，在同一切片内继续；不强制向用户汇报，也不自动开启下一切片 | 人工 + 无用户闸门反向探针 |
| S43 | 用户给出清楚、同范围的体验反馈或窄修改意见 | 原 owner TASK 保持责任，更新同一切片的候选代次并继续；不为同范围反馈重复确认、另建 replacement TASK 或新切片 | 人工 + 返工状态机 |
| S44 | 用户反馈含义不清，或修改会改变体验方向、范围、路线、成本、安全、隐私、发布边界 | 统筹说明具体影响并只问必要选择；未确认前不把建议写成授权，也不继续受影响的步骤 | 人工 + 授权轴 |
| S45 | 用户只是追问状态、澄清一句话或进行普通对话 | 直接自然回答，不机械套三段标题；但若答案同时要求用户体验、补信息或做决定，仍须明确写出用户动作 | 人工 + 过度汇报反向探针 |
| S46 | 统筹正式向用户交体验、选择、风险或重要阶段收口 | 信息骨架稳定，篇幅按情况伸缩：必须有 `结果` 和 `需要你做什么`，只有真实限制或风险时才写 `还需注意`；不得为凑格式制造空话 | 人工 + 长会话格式漂移探针 |
| S47 | 用户发来未被规则逐项列出的插话、问题、反馈或请求 | 不靠关键词套场景；先识别真实意图、对当前 TASK 的影响，以及是否需要用户独有信息、体验判断或授权。无实质影响就直接回应并保持任务现场，有影响再按对应用户闸门处理 | 人工 + 未枚举场景泛化探针 |
