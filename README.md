...
`2.0.1` 修复两类临时任务收口问题：用户明确放弃后，正常清理或崩溃恢复会让普通 TASK 进入待统筹核收，同时保留“未交付、未集成、未发布”的事实边界；临时资源清理和真实会话归档改用两张独立收据。宿主有自动归档能力时自动完成并继续；缺少自动能力时暂停并提醒用户归档具体会话，只有用户回复“我已将该会话归档”后才继续。
